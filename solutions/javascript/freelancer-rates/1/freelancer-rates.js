...
export function monthRate(ratePerHour, discount) {
  ...
  let month = day * 22;
  let discountedRate = applyDiscount(month, discount);
  let roundedUpDays = Math.ceil(discountedRate);
...
export function daysInBudget(budget, ratePerHour, discount) {
  let day = dayRate(ratePerHour);
  let discountedRate = applyDiscount(day, discount);
  let roundedDownDays = Math.floor(budget/ discountedRate);
  return roundedDownDays;
}
