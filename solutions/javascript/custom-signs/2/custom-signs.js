...
export function buildBirthdaySign(age) {
  const ageComment = age >= 50 ? `mature` : `young`;
  return `Happy Birthday! What a ${ageComment} fellow you are.`;
}
...
export function costOf(sign, currency) {
  const numberOfLetters = sign.length;

  const totalPrice = (20 + numberOfLetters * 2).toFixed(2);
