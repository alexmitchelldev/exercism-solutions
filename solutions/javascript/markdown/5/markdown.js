...
class Helpers {
  ...
  handleList(line, lines, html, isList) {
    ...
    html = (isLastLine && isList) ? `${html}</ul>` : html;
...
class Parsers {
  ...
  parseLine(markdown) {
    let parsedHtml = this.parseParagraph(markdown);
...
class MarkdownParser {
  ...
  parse() {
    ...
    for (const line of lines) {
      ...
      [lineHtml, isList] = helpers.handleList(line, lines, lineHtml, isList);
      html += lineHtml;
    }
...
// initialise parsedHtml as call to this.parseParagraph and remove unnecessary 'else' logic from parseLine function