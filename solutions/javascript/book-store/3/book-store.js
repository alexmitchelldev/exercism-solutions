...
const findBestDiscount = bookSetsObject => {
  ...
  for (let i = 0; i < keys.length; i++) {
    for (let j = i + 1; j < keys.length; j++) {
        if (((calculateDiscount(bookSetsObject[keys[i]].length - 1) * BOOK_PRICE) + (calculateDiscount(bookSetsObject[keys[j]].length + 1) * BOOK_PRICE)) < ((calculateDiscount(bookSetsObject[keys[i]].length) * BOOK_PRICE) + (calculateDiscount(bookSetsObject[keys[j]].length) * BOOK_PRICE))) {
