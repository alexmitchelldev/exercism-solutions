...
const findBestDiscount = bookSetsObject => {
  ...
  for (let i = 0; i < keys.length; i++) {
    for (let j = i + 1; j < keys.length; j++) {
        if (((calculateDiscount(bookSetsObject[keys[i]].length - 1) * 800) + (calculateDiscount(bookSetsObject[keys[j]].length + 1) * 800)) < ((calculateDiscount(bookSetsObject[keys[i]].length) * 800) + (calculateDiscount(bookSetsObject[keys[j]].length) * 800))) {
...
export const cost = (books) => {
  ...
  for (const bookSet in bookSetsObject) {
    total += ((bookSetsObject[bookSet].length * 800) * calculateDiscount(bookSetsObject[bookSet].length));
  }
